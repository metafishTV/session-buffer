...
- Check the registry for reserved ranges. Never assign to reserved numbers.
...
Project-specific behavioral directives (reserved numbering ranges, terminology constraints, custom voice rules) are generated by `/distill:differentiate` and stored in the project repo at `.claude/distill.config.md`. If that file exists, its directives supplement the generic directives above.

...
1. **Forward note collision prevention**: Read `<repo>/.claude/skills/distill/forward_notes.json` if it exists. Use `next_number` for any new forward note candidates. Never assign a number that already appears in the registry. If the registry does not exist, start at the default (`next_number: 1`). If `.claude/distill.config.md` specifies reserved ranges, respect them.
   - **CRITICAL**: NEVER write a registry entry keyed to a number within a reserved range. Reserved ranges belong to pre-existing project content, not the distillation registry. If a distillation reveals that an existing section should be amended or cross-referenced, note it in the interpretation prose as: `> Cross-ref: §[N] — [reason]`. Only genuinely NEW theoretical candidates get registry entries, always numbered from `next_number`.
